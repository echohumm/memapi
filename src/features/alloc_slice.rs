...
pub trait AllocSlice: Alloc {
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the slice is zero-sized.
    #[cfg_attr(miri, track_caller)]
    fn salloc_copy<T: Copy>(&self, data: &[T]) -> Result<NonNull<[T]>, AllocError> {
        // SAFETY: `T: Copy`
        unsafe { self.salloc_copy_unchecked(data) }
    }
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the slice is zero-sized.
    #[cfg_attr(miri, track_caller)]
    unsafe fn salloc_copy_unchecked<T>(&self, data: &[T]) -> Result<NonNull<[T]>, AllocError> {
        alloc_then(self, data.layout(), data, |p, data| {
            let p = p.cast::<T>();
    ...
    #[track_caller]
    #[inline]
    unsafe fn isalloc<T, I: Fn(NonNull<[T]>, &mut usize)>(
        &self,
        len: usize,
        init: I,
    ) -> Result<NonNull<[T]>, AllocError> {
        let mut guard = SliceAllocGuard::new(tri!(do self.salloc::<T>(len)).cast::<T>(), self, len);
    ...
    #[track_caller]
    #[inline]
    fn salloc_def<T: Default>(&self, len: usize) -> Result<NonNull<[T]>, AllocError> {
        self.salloc_with(len, |_| T::default())
    }
    ...
    /// - be properly aligned
    #[cfg_attr(miri, track_caller)]
    unsafe fn zdealloc_n<T>(&self, ptr: NonNull<T>, n: usize) {
    ...
    #[cfg_attr(miri, track_caller)]
    #[inline]
    fn gsalloc<T>(&'_ self, len: usize) -> Result<SliceAllocGuard<'_, T, Self>, AllocError> {
        match self.salloc::<T>(len) {
...
// TODO: split these up better

#[cfg(feature = "alloc_slice_extend")]
/// Slice-extending extension methods for [`AllocSlice`], providing convenient functions for
/// extending slices.
pub trait AllocExtendSlice: AllocSlice {
    ...
    /// - `extra` must point to a valid slice.
    ///
    /// # Errors
    ///
    /// - [`AllocError::AllocFailed`] if allocation fails.
    ...
    unsafe fn extend_slice<T: Clone>(
        ...
        self.extend_raw(slice.cast::<T>(), init, len, extra.cast::<T>(), ext_len)
    ...
    /// - `slice` must point to a slice allocated with this allocator.
    ///   - `init` must be the number of initialized elements in that slice.
    /// - `extra_ptr` must point to a valid slice.
    ///   - `extra_len` must be the number of initialized elements in that slice.
    ///
    /// # Errors
    ///
    /// - [`AllocError::AllocFailed`] if allocation fails.
    ...
    unsafe fn extend_raw<T: Clone>(
                ...
                impl<T: Clone, A: AllocSlice + ?Sized> SpecExtendSlice<T, A> for [T] {
                    $($extra_token)? fn dupe_into(
                        ...
                        unsafe {
                            ...
                            // we use forget instead of release because we don't want the return value
        ...
        let new_ptr = tri!(do self.grow_raw(ptr, len, len + extra_len));
...
#[cfg(feature = "alloc_slice_extend")]
impl<A: AllocSlice + ?Sized> AllocExtendSlice for A {}
...
pub trait AllocSliceExt: AllocSlice + crate::AllocExt {
    ...
    #[track_caller]
    #[inline]
    unsafe fn drop_zdealloc_slice<T>(
        &self,
        slice: NonNull<[core::mem::MaybeUninit<T>]>,
        init: usize,
    ) {
        self.drop_zdealloc_raw(slice.cast::<T>(), init, nonnull_slice_len(slice));
    }
    ...
    #[track_caller]
    #[inline]
    unsafe fn drop_zdealloc_raw<T>(&self, slice: NonNull<T>, init: usize, len: usize) {
        ptr::drop_in_place(crate::helpers::slice_ptr_from_raw_parts(
            slice.as_ptr(),
            init,
        ));
        self.zdealloc_n(slice, len);
    }
    ...
    #[cfg_attr(miri, track_caller)]
    #[inline]
    unsafe fn drop_dealloc_slice<T>(&self, ptr: NonNull<[core::mem::MaybeUninit<T>]>, init: usize) {
        self.drop_dealloc_raw(ptr.cast::<T>(), init, nonnull_slice_len(ptr));
    }
    ...
    #[track_caller]
    #[inline]
    unsafe fn drop_dealloc_raw<T>(&self, ptr: NonNull<T>, init: usize, len: usize) {
    ...
    /// - [`AllocError::GrowSmallerNewLayout`] if `new_len < slice.len()`.
    #[track_caller]
    unsafe fn isgrow<T, I: Fn(NonNull<[T]>, &mut usize)>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        init: I,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.igrow_raw(slice.cast::<T>(), nonnull_slice_len(slice), new_len, init)
            .map(|p| nonnull_slice_from_raw_parts(p, new_len))
    }
    ...
    ///
    #[track_caller]
    unsafe fn igrow_raw<T, I: Fn(NonNull<[T]>, &mut usize)>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
        ...
        resize_init(
            ...
            len,
            new_len,
            |a, p, len, new| a.grow_raw(p, len, new),
            init,
        )
    }
    ...
    /// - [`AllocError::GrowSmallerNewLayout`] if `new_len < slice.len()`.
    #[track_caller]
    unsafe fn sgrow_def<T: Default>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.grow_raw_def(slice.cast::<T>(), nonnull_slice_len(slice), new_len)
            .map(|p| nonnull_slice_from_raw_parts(p, new_len))
    }
    ...
    /// - [`AllocError::GrowSmallerNewLayout`] if `new_len < len`.
    #[track_caller]
    unsafe fn grow_raw_def<T: Default>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
    ) -> Result<NonNull<T>, AllocError> {
        self.grow_raw_with(ptr, len, new_len, |_| T::default())
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn sgrow_copy_from<T: Copy>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        src: NonNull<[T]>,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.sgrow_copy_from_unchecked(slice, new_len, src)
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn sgrow_copy_from_unchecked<T>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        src: NonNull<[T]>,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.grow_raw_copy_from_unchecked(
            slice.cast::<T>(),
            nonnull_slice_len(slice),
            new_len,
            src.cast::<T>(),
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn grow_raw_copy_from<T: Copy>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
        ...
        self.grow_raw_copy_from_unchecked(ptr, len, new_len, src, src_len)
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn grow_raw_copy_from_unchecked<T>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
        ...
        let new_ptr = tri!(do self.grow_raw(ptr, len, new_len));
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the computed slice would be zero-sized.
    #[track_caller]
    unsafe fn resalloc_init<T, I: Fn(NonNull<[T]>, &mut usize)>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        init: I,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.realloc_raw_init(slice.cast::<T>(), nonnull_slice_len(slice), new_len, init)
            .map(|p| nonnull_slice_from_raw_parts(p, new_len))
    }
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the computed slice would be zero-sized.
    #[track_caller]
    unsafe fn realloc_raw_init<T, I: Fn(NonNull<[T]>, &mut usize)>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
        ...
        resize_init(
            ...
            len,
            new_len,
            |a, p, len, new| a.realloc_raw(p, len, new),
            init,
        )
    }
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the computed slice would be zero-sized.
    #[track_caller]
    unsafe fn resalloc_def<T: Default>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.realloc_raw_def(slice.cast::<T>(), nonnull_slice_len(slice), new_len)
            .map(|p| nonnull_slice_from_raw_parts(p, new_len))
    }
    ...
    /// - [`AllocError::ZeroSizedLayout`] if the computed slice has a size of zero.
    #[track_caller]
    unsafe fn realloc_raw_def<T: Default>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
    ) -> Result<NonNull<T>, AllocError> {
        self.realloc_raw_with(ptr, len, new_len, |_| T::default())
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn resalloc_copy_from<T: Copy>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        src: NonNull<[T]>,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.resalloc_copy_from_unchecked(slice, new_len, src)
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn resalloc_copy_from_unchecked<T>(
        &self,
        slice: NonNull<[T]>,
        new_len: usize,
        src: NonNull<[T]>,
    ) -> Result<NonNull<[T]>, AllocError> {
        self.realloc_raw_copy_from_unchecked(
            slice.cast::<T>(),
            nonnull_slice_len(slice),
            new_len,
            src.cast::<T>(),
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn realloc_raw_copy_from<T: Copy>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
        ...
        self.realloc_raw_copy_from_unchecked(ptr, len, new_len, src, src_len)
    }
    ...
    ///   `src_len < new_len - len`.
    #[track_caller]
    unsafe fn realloc_raw_copy_from_unchecked<T>(
        &self,
        ptr: NonNull<T>,
        len: usize,
        new_len: usize,
