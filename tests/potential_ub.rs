...
fn slice_pointer_from_raw_parts_works() {
    match sp_frp_inner() {
        ...
        0 => (),
        1 => {
            panic!("slice_ptr_from_raw_parts() test 1 failed: result doesn't dereference properly")
        }
        2 => {
            panic!("slice_ptr_from_raw_parts() test 2 failed: result doesn't have the same pointer")
        }
        3 => {
            panic!("slice_ptr_from_raw_parts() test 3 failed: result doesn't have the same length")
        }
        4 => panic!(
            "slice_ptr_from_raw_parts() test 4 failed: result as nonnull doesn't have the same \
             length"
        ),
        5 => {
            panic!("slice_ptr_from_raw_parts() test 5 failed: result doesn't have the same values")
        }
        6 => panic!(
            "slice_ptr_from_raw_parts() test 6 failed: result doesn't have the correct values"
        ),
...
fn sp_frp_inner() -> usize {
    ...
    unsafe {
        if len != nonnull_slice_len(nonnull_slice_from_raw_parts(NonNull::new_unchecked(ptr), len))
        {
            return 4;
        }
    }
